...
def batch_autocorr(data, lag, starts, ends, threshold, backoffset=0):
    ...
    """
    # This value is total number of rows i.e 145036 pages.
    n_series = data.shape[0]
    # This is total number of columns. So a dataframe has a thing called shape which is an list of 2 items. [rows, columns]
    n_days = data.shape[1]
...
def traffic_median(df: pd.DataFrame, median_length: int) -> pd.DataFrame:
    print("Median length: " + str(median_length))
    df2 = pd.DataFrame(index=df.index)
    for i in range(0, int(df.shape[1]/median_length)):
        mn = df.iloc[:, i*median_length:(i+1)*median_length].median(axis=1)
        df2 = pd.concat([df2, mn], axis=1)
